...
export default function OrcamentoDetalhePage() {
  ...
  const updateStatus = api.budget.updateStatus.useMutation({
    onSuccess: async () => {
      await utils.budget.getById.invalidate({ budgetId: id });
      await utils.budget.list.invalidate();
    },
  });

  const createFromBudget = api.serviceOrder.createFromBudget.useMutation({
    onSuccess: (data) => {
  ...
  async function handleSendWhatsApp() {
    ...
    const message = [
      ...
      budget.notes ? `\nObs: ${budget.notes}` : "",
      "",
      "O PDF do orçamento está em anexo.",
      "",
    ...
    const blob = getBudgetPdfBlob(data);
    const fileName = `orcamento-${budget.number.replace(/\s/g, "-")}.pdf`;
    ...
    const whatsAppUrl = buildWhatsAppUrl(
      budget.client.phone,
      `${message}\n\n(O PDF foi baixado. Por favor, anexe-o neste chat.)`,
    );
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-4">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex gap-2">
            <Button
              variant="secondary"
              onClick={() => updateStatus.mutate({ budgetId: id, status: "APPROVED" })}
              disabled={updateStatus.isPending || budget.status === "APPROVED"}
            >
              Aprovar
            </Button>
            <Button
              variant="secondary"
              onClick={() => updateStatus.mutate({ budgetId: id, status: "REJECTED" })}
              disabled={updateStatus.isPending || budget.status === "REJECTED"}
            >
              Recusar
            </Button>
          </div>
          {budget.status === "APPROVED" && !budget.serviceOrderId ? (
            <Button
              onClick={() => createFromBudget.mutate({ budgetId: id })}
              disabled={createFromBudget.isPending}
            >
              {createFromBudget.isPending ? "Gerando..." : "Gerar Ordem de Serviço"}
            </Button>
          ) : null}
        </div>
        {updateStatus.error ? (
        ...
        {createFromBudget.error ? (
          <p className="mt-2 text-sm text-red-600">{createFromBudget.error.message}</p>
        ) : null}
      </section>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-6">
        ...
        {budget.serviceOrder ? (
          <div className="mb-4">
            ...
            <Link
              href={`/ordens-servico/${budget.serviceOrder.id}`}
