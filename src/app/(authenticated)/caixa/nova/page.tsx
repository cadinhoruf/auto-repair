...
export default function NovaMovimentacaoPage() {
  ...
  useEffect(() => {
    if (!preselectedOSId || !selectedOS || hasPrefilledRef.current) return;
    const valor = Number(selectedOS.finalValue ?? selectedOS.estimatedValue ?? 0);
    if (valor <= 0) return;
    hasPrefilledRef.current = true;
    reset(
      {
        type: "IN",
        description: `Serviço OS - ${selectedOS.client.name}`,
        value: String(valor),
        date: new Date().toISOString().slice(0, 10),
        serviceOrderId: preselectedOSId,
        installmentsCount: 1,
